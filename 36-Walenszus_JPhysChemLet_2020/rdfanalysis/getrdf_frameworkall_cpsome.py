...
for i in [10,50,100, 180]:
    rdf1s = []
    rdf2s = []
    rdf3s = []
    rdf4s = []
    for j in [1,2,3,4,5]:
        with open("../rawdata/dut49_cp/loading_"+str(i)+"_"+str(j)+"/tmp_"+str(j)+".rdf", "r") as f:
            lines = f.read().split('\n')
    ...
    ax1.plot(rad,mean_rdf1s, color=colors[count], label=str(i))
    count=count+1

ax2.plot(rad,mean_rdf1s, color=colors[1], label=str(i))
ax3.plot(rad,mean_rdf1s, color=colors[1], label=str(i))
plt.xlabel('$r$ / Å')
plt.ylabel('g($r$)')
